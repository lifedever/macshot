...
- Settings controls that SwiftUI has no equivalent for — shortcut recording, colour wells, the SF Symbol picker — stay as AppKit views wrapped in `NSViewRepresentable` rather than being reimplemented. A list of rows is not one of those: the menu-order list is plain SwiftUI inside the form's own section, because an AppKit list bridged in brings its own bordered box (a frame inside the form's card) and bezelled buttons that macOS tints with the accent colour.
- **No settings pane may scroll.** Every pane sets `.scrollDisabled(true)`, and `sizeWindowToPane` caps the window at the screen's visible height minus 120 — so a pane taller than that is not scrolled, it is *cut off*. Budget 680pt, what a 13-inch display allows. When a pane outgrows it, restructure rather than let it overflow: independent on/off settings go into two columns of checkboxes (Capture, Tools), paired label+control rows into two columns (Shortcuts), or the pane splits. Measure, don't estimate — render the pane into an `NSHostingView` with `sizingOptions = [.intrinsicContentSize]` and read its `intrinsicContentSize.height`.
...
- **Read the library original, publish the copy.** A published file sits outside the sandbox and is reachable only through the security-scoped lease the save holds — and that lease dies with the save. Anything `async` that touches it afterwards (generating a recording's poster frame, a deferred upload) finds it unreadable and usually fails silently. Everything macshot opens itself works from the library original in the container; only Finder, which needs no scope from us, is pointed at the published path. This reproduces solely in the signed sandboxed build — a command-line probe reads the same file fine.
- **One save folder, one filename template.** `SaveDirectoryAccess` and `FilenameFormatter` each expose exactly one, shared by screenshots and recordings. Recordings used to carry their own pair, so a file's name and location depended on which of two panes had been edited last, and the folder set under Output silently did nothing for two of the three "When done" options. Do not add a per-capture-kind override.
