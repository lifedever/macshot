...
class DetachedEditorWindowController: NSObject, NSWindowDelegate {
    ...
    private func saveToHistory(_ save: HistorySave, completion: ((Bool) -> Void)? = nil) {
        ...
        guard history.maxEntries > 0 else {
            ...
            historyEntryID = nil
            // Saving reports where the file landed; history only reports success.
            ImageSaveService.save(finalImage, sheetWindow: window) { url in
                if let url { AppDelegate.showSavedToast(for: url) }
                finished(url != nil)
            }
            return
        }
...
extension DetachedEditorWindowController: OverlayViewDelegate {
    ...
    func overlayViewDidRequestSaveAs() {
        guard let save = captureHistorySave() else { return }
        ImageSaveService.showSavePanel(for: save.image, sheetWindow: window) { [weak self] url in
    ...
    func overlayViewDidRequestFileSave() {
        guard let save = captureHistorySave() else { return }
        ImageSaveService.saveToConfiguredFolder(save.image, sheetWindow: window) { [weak self] url in
            guard let url else { return }
            self?.playCopySound()
            AppDelegate.showSavedToast(for: url)
            self?.autoSaveToHistoryIfNeeded(save)
